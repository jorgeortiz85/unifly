...
(() => {
  ...
  const snippet = (body, terms) => {
    ...
    for (const t of terms) {
      const i = lo.indexOf(t.toLowerCase());
      if (i !== -1) {
        pos = i;
        break;
      }
    ...
    const start = Math.max(0, pos - 60),
      end = Math.min(body.length, start + 120);
  ...
  const render = (hits, terms, el) => {
    ...
    if (!hits.length) {
      el.innerHTML =
        '<div class="search-result search-result--empty">No results found</div>';
      return;
    }
    el.innerHTML = hits
      .slice(0, 10)
      .map((r) => {
        ...
        const sec = section
          ? `<div class="search-result__section">${esc(section)}</div>`
          : "";
...
${sec}<div class="search-result__snippet">${snippet(body || "", terms)}</div></a>`;
  ...
  const setActive = (container, idx) => {
    const items = container.querySelectorAll(
      ".search-result:not(.search-result--empty)",
    );
  ...
  const init = () => {
    ...
    input.addEventListener("input", () => {
      ...
      debounceTimer = setTimeout(async () => {
        try {
          ...
          results.innerHTML =
            '<div class="search-result search-result--empty">Search index unavailable</div>';
          return;
        }
    ...
    input.addEventListener("keydown", (e) => {
      ...
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setActive(results, activeIdx + 1);
      } else if (e.key === "ArrowUp") {
        e.preventDefault();
        setActive(results, activeIdx - 1);
      } else if (e.key === "Enter" && activeIdx >= 0) {
        e.preventDefault();
        if (items[activeIdx]?.href)
