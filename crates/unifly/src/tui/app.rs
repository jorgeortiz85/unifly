...
use unifly_api::{Command, Controller, EntityId, MacAddress};
...
pub struct App {
    ...
    stats_period: crate::tui::action::StatsPeriod,
}
...
impl App {
    ...
    pub fn new(controller: Option<Controller>) -> Self {
        ...
        Self {
            ...
            stats_period: crate::tui::action::StatsPeriod::default(),
        }
    }
                ...
                if *target != self.active_screen {
                    ...
                    if *target == ScreenId::Stats {
                        self.action_tx.send(Action::RequestStats(
                            crate::tui::action::StatsPeriod::default(),
                        ))?;
                    }
                }
            }
    ...
    fn fetch_stats(&self, period: crate::tui::action::StatsPeriod) {
        use std::sync::atomic::Ordering;

        use crate::tui::action::StatsData;
    ...
    fn render_notification(&self, frame: &mut Frame, area: Rect, notif: &Notification) {
        use crate::tui::action::NotificationLevel;
