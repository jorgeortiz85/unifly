...
use unifly_api::{Client, Device, DeviceType, Event, HealthSummary, Network};
...
impl DashboardScreen {
    ...
    fn render_system_health(&self, frame: &mut Frame, area: Rect) {
        ...
        let wireless_clients = self
            .clients
            .iter()
            .filter(|c| c.client_type == unifly_api::ClientType::Wireless)
            .count();
        let wired_clients = self
            .clients
            .iter()
            .filter(|c| c.client_type == unifly_api::ClientType::Wired)
            .count();
