    ...
    match args.command {
        ...
        AclCommand::Create {
            ...
            let req = if let Some(ref path) = from_file {
                ...
                unifly_api::command::CreateAclRuleRequest {
                    name: name.unwrap_or_default(),
                    rule_type: rule_type.as_ref().map_or("IPV4".into(), |rt| match rt {
                        crate::cli::args::AclRuleType::Ipv4 => "IPV4".into(),
                        crate::cli::args::AclRuleType::Mac => "DEVICE".into(),
                    }),
                    action: match action {
                        Some(crate::cli::args::AclAction::Allow) => {
                            unifly_api::model::FirewallAction::Allow
                        }
                        Some(crate::cli::args::AclAction::Block) | None => {
                            unifly_api::model::FirewallAction::Block
                        }
                    },
        ...
        AclCommand::Update { id, from_file } => {
            ...
            let update = if let Some(ref path) = from_file {
                ...
                unifly_api::command::UpdateAclRuleRequest::default()
            };
