...
mod tests {
    use super::{
        CreateAclRuleRequest, CreateFirewallPolicyRequest, TrafficFilterSpec, UpdateAclRuleRequest,
    ...
    fn resolve_filters_combines_dst_ip_and_dst_port() {
        ...
        match &req.destination_filter {
            Some(TrafficFilterSpec::IpAddress {
                ...
                assert_eq!(ports.as_deref(), Some(&["80".to_owned()][..]));
            }
    ...
    fn resolve_filters_update_request_works() {
        ...
        req.resolve_filters().expect("should resolve");
        match &req.destination_filter {
            Some(TrafficFilterSpec::Port { ports, .. }) => {
                assert_eq!(ports, &["443", "8443"]);
            }
