...
use crate::model::{
    ...
    device::{
        Device, DeviceState, DeviceStats, DeviceType, PoeInfo, Port, PortConnector, PortState,
        Radio,
    },
...
// ── Interface parsing (ports + radios) ────────────────────────────
...
/// Parse ports from Integration API `interfaces.ports[]`.
fn parse_integration_ports(interfaces: &Value) -> Vec<Port> {
...
/// Parse radios from Integration API `interfaces.radios[]`.
fn parse_integration_radios(interfaces: &Value) -> Vec<Radio> {
...
/// Merge `txRetriesPct` from the statistics `interfaces.radios[]` into existing radios.
...
/// Parse ports from Session API `port_table` in the device extra map.
fn parse_session_ports(extra: &serde_json::Map<String, Value>) -> Vec<Port> {
...
/// Map Session API radio band identifier to frequency in GHz.
...
/// Parse radios from Session API `radio_table` in the device extra map.
fn parse_session_radios(extra: &serde_json::Map<String, Value>) -> Vec<Radio> {
